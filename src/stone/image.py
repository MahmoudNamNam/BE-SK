...
# Coarse label mapping: palette name -> list of coarse labels per palette index
# Maps fine-grained labels to Light, Medium, Dark
COARSE_LABEL_MAPPING = {
    "perla": ["Dark"] * 5 + ["Medium"] * 3 + ["Light"] * 3,  # 11 colors: 0-4 Dark (#81654F included), 5-7 Medium, 8-10 Light
    "monk": ["Light"] * 4 + ["Medium"] * 3 + ["Dark"] * 3,  # 10 colors: 0-3 Light, 4-6 Medium, 7-9 Dark (monk is reversed: lightest first)
    "yadon-ostfeld": ["Dark"] * 3 + ["Medium"] * 4 + ["Light"] * 3,  # 10 colors: 0-2 Dark, 3-6 Medium, 7-9 Light
    "proder": ["Dark"] * 4 + ["Medium"] * 4 + ["Light"] * 3,  # 11 colors: 0-3 Dark, 4-7 Medium, 8-10 Light
    "bw": ["Light"] * 6 + ["Medium"] * 6 + ["Dark"] * 5,  # 17 colors: 0-5 Light (white), 6-11 Medium (gray), 12-16 Dark (black)
}
...
def tone_label_to_coarse(tone_id: int, palette_name: str) -> str:
    """
    Convert a palette index (tone_id) to a coarse label (Light, Medium, Dark).
    
    :param tone_id: The index of the matched palette color (0-based)
    :param palette_name: The name of the palette (e.g., 'perla', 'monk', 'yadon-ostfeld', 'proder', 'bw')
    :return: Coarse label: "Light", "Medium", or "Dark"
    """
    # Normalize palette name (handle aliases)
    full_palette = build_full_palette()
    normalized_name = None
    for name in DEFAULT_TONE_PALETTE.keys():
        if palette_name in [name] + TONE_ALIAS.get(name, []):
            normalized_name = name
            break
    
    if normalized_name and normalized_name in COARSE_LABEL_MAPPING:
        mapping = COARSE_LABEL_MAPPING[normalized_name]
        if 0 <= tone_id < len(mapping):
            return mapping[tone_id]
    
    # Fallback: if palette not in mapping or tone_id out of range, return None
    return None
...
def classify(
    ...
    :param palette_name: Name of the palette (for coarse label mapping)
    :return:
    """
    detect_skin_fn = detect_skin_in_bw if is_bw else detect_skin_in_color
    ...
    # Add coarse tone label if palette_name is provided
    if palette_name:
        coarse_label = tone_label_to_coarse(tone_id, palette_name)
        if coarse_label:
            result["coarse_tone_label"] = coarse_label
