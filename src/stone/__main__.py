...
def main():
    ...
    header = (
        ...
        + ",skin tone,tone label,coarse tone label,accuracy(0-100)"
    )
        ...
        with tqdm(image_paths, desc="Processing images", unit="images") as pbar:
            for result in pool.imap(process_wrapper, image_paths):
                ...
                for face_record in faces:
                    ...
                    # Get coarse tone label if available, otherwise empty string
                    coarse_label = face_record.get("coarse_tone_label", "")
                    record = (
                        [f"{basename}{extension}", image_type, face_id]
                        + np.hstack(dominant_colors).tolist()
                        + [face_record["skin_tone"], face_record["tone_label"], coarse_label, face_record["accuracy"]]
                    )
